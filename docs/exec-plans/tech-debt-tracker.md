...
- [ ] **Register INV-001: parser bounded-allocation contract.** Bounded-decode guards on the JSON-Schema-lite parser are tracked as F12 (PRD `dispatchschema-bounded-allocation`); delete the structural-cap half of this entry when F12 lands. The remaining work is the INV-### registration itself: CLAUDE.md has no `## Safety Rules` section, so no invariant ID can yet be cited. Adding the section + registering INV-001 (parser bounded-allocation contract: `enum[]`, `required[]`, property count, and schema byte size all bounded) is a separate cross-cutting chore and remains open after F12.
...
- [ ] **F04 MINOR DRY: hardcoded dispatch-error prefix in `server.go`.** `internal/stdiomcp/server.go:113` hardcodes `"roundtable dispatch error: invalid schema parameter:"` in a single `fmt.Errorf` wrapper. If the prefix changes (e.g., to match a forge-side log convention), the literal must be hand-edited. Fix: extract a package-level const, OR consolidate via the existing dispatch-error envelope so the inner wrapper supplies only the leaf message. Code-reviewer MINOR, non-blocking.

- [ ] **F04 MINOR DoS: no byte cap on `input.Schema`, no enum-length cap.** Tracked as F12 (PRD `dispatchschema-bounded-allocation`); contract specifies `MaxSchemaBytes=65536` at the MCP boundary plus `MaxEnumEntries=256`, `MaxRequiredEntries=256`, `MaxProperties=256` inside `Parse`. Delete this entry when F12 lands.
...
- [ ] **F12 HIGH: MCP SDK pre-handler argument validation can allocate before `SafeParse` cap fires.** `internal/stdiomcp/server.go` registers tools via generic `mcp.AddTool`, which causes the SDK to unmarshal `req.Params.Arguments` into `map[string]any` and validate/remarshal BEFORE `ToolInput.Schema` reaches `SafeParse`. A multi-MB `schema` field could force pre-handler allocation/CPU regardless of F12's 64 KB cap. Bounded by MCP transport-level body cap (typically MB-scale). Fix: switch schema-bearing tools to a low-level MCP handler that extracts `schema` from raw arguments with a size-bounded decoder, OR enforce a transport-level request cap. Roundtable landing review attempt 1 finding (codex HIGH).

- [ ] **F12 MEDIUM: `dispatchschema.Parse` remains exported as a footgun.** Future callers can pass external bytes directly to `Parse` and bypass `SafeParse`'s byte cap. Mitigated by godoc warning per arch-advisor + designer; unexporting (rename to `parse`) breaks the parser package's count-cap unit tests that need to hit `Parse` without the byte cap. Fix (opportunistic): refactor count-cap tests to call through `SafeParse` with under-cap fixtures, then unexport `Parse`. Roundtable landing review attempt 1 finding (gemini, kimi, minimax).

- [ ] **F12 LOW: `errAs` helper in `dispatchschema/schema.go:492` should use `errors.As`.** Hand-rolled type assertion exists to avoid importing `errors`. Now that `Unwrap()` is in the package surface and `errors.As` is the public discrimination contract for callers, importing `errors` and replacing `errAs` with `errors.As` would be more idiomatic. Already noted in the F01 era tech-debt entry but bumped in priority post-F12. Roundtable landing review attempt 1 finding (claude).
